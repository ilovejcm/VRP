...
##            print self.route
            point_list_temp = [j.id for j in point_list]
            
##            print "new :"
##            print pltemp
            cent = i.chose_start_center(pltemp)
            pltemp.remove(cent)
##            print set(point_list_temp) - set(self.route)
            if set(point_list_temp) - set(self.route) == set([]):
                print 'i am break'
                break
            if set(point_list_temp) - set(self.route) <= set([0,1,2,3,4,5]):
                print 'i am break'
                break
            ...
            while (i.time < Static.LAST_TIME):
                ...
                while (i.weight < Static.MAX_WEIGHT_FOR_CAR):
                    ...
                    print i.weight
                    if i.weight > Static.MAX_WEIGHT_FOR_CAR:
                        i.weight -= point_list[one].weight
                        break
                    i.route.append(one)
                    print i.route                    
                    temp_time += point_list[i.route[len(i.route)-2]].work_time + Static.distance(i.route[len(i.route)-2],i.route[len(i.route)-1],point_list) / Static.CAR_SPEED
                        
                #i.route.pop()
                if i.route[len(i.route)-1] in [0,1,2,3,4,5]:
                    for j in i.route:
                        self.route.append(j)
                    break
                ...
                if home == -1:
                    ...
                    center_new_list = range(0,Static.HAS_CENTER)
                    i.chose_start_center(center_new_list)
                    i.weight = 0
